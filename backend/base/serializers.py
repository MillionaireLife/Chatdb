...
class queryresponseSerializer(serializers.ModelSerializer):
    ...
    # add "validate_" before the field name to validate the field, here validate_response is used to validate "response" field. If field doesn't exist then the validate_field won't be called.
    ...
    def validate_table(self, value):
        ...
        if len(value["body"]) > 0 and len(value["head"]) != len(value["body"][0]):
            raise serializers.ValidationError(
                "Number of columns in head and body should be the same"
            )
